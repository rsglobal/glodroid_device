...
$(CONFIGURE_TARGET): $(PATCH_TARGET)
	...
	@make -C $(OUT_SRC_DIR) O=$(OUT_BUILD_DIR) defconfig 2>&1 > $@.tmp || (cat $@.tmp && exit 1)
	@cp configs/defconfig $(OUT_BUILD_DIR)/.config  2>&1 >> $@.tmp || (cat $@.tmp && exit 1)
	@$(KMAKE) olddefconfig 2>&1 >> $@.tmp || (cat $@.tmp && exit 1)
	@$(OUT_SRC_DIR)/scripts/kconfig/merge_config.sh -m -O $(OUT_BUILD_DIR)/ $(OUT_BUILD_DIR)/.config $(KCONFIG_FRAGMENTS) 2>&1 >> $@.tmp || (cat $@.tmp && exit 1)
	@$(KMAKE) olddefconfig 2>&1 >> $@.tmp || (cat $@.tmp && exit 1)
	@mv $@.tmp $@ -f
...
$(BUILD_TARGET): $(CONFIGURE_TARGET)
	@echo Building...
	@mkdir -p $(OUT_BUILD_DIR)
	@$(KMAKE) dtbs all -j$(NUMPROC) 2>&1 > $@.tmp || (cat $@.tmp && exit 1)
	@mv $@.tmp $@ -f
...
ifeq ($(TARGET_ARCH),arm64)
	@lz4c -f -c1 $(OUT_INSTALL_DIR)/vmlinuz $(OUT_INSTALL_DIR)/kernel
else
